...
export class SqsProvider {
    ...
    async sendMessage(messageBody: string, attributes?: MessageBodyAttributeMap): Promise<void> {
        ...
        const result = await this.sqs.sendMessage(params).promise().catch((e: any) => console.log(e.message));
        if (!result) { return }
    ...
    async receiveMessages(count?: number): Promise<Message[]> {
        ...
        const result = await this.sqs.receiveMessage(params).promise().catch((e: any) => console.log(e.message));
        if (!result) { return }
    ...
    async deleteMessage(receiptHandle: string) {
        ...
        await this.sqs.deleteMessage(deleteParams).promise().catch((e: any) => console.log(e.message))
    }
}
